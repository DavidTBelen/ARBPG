...
"With bar tau adaptive each iteration"
def RNBPG_efficient(U0,V0,inst,btau,tau_min,tau_max,sigma,beta,gap=50,prec=1e-6,stop_rule=0,max_stop=1000):
        ...
        if ik < m:
            ...
            while adaptive_ls:
                ...
                if norm(di) != 0:
                    ...
                    temp_arg = 1/2*norm(dV)**2 - np.dot(AUV[ik,:],dV)
                                        
                    f_eval += 1
                    if temp_arg <= - sigma*norm(di)**2:
        ...
        # Update V:
            ...
            while adaptive_ls:
                ...
                if norm(di) != 0:
                    ...
                    temp_arg = 1/2*norm(dU)**2 - np.dot(AUV[:,jk],dU)
                    
                    f_eval += 1
                    if temp_arg <=  - sigma*norm(d)**2:
            ...
            while adaptive_ls:
                ...
                if norm(di) != 0:
                    ...
                    if FVhat <= F_list[-1] - sigma*norm(d)**2:
                        ...
                        # Once the adaptive linesearch finished we do the boosted one
                        lam = blam
                        b_counter = 0
...
"ARBPG with l0 sparse constraint"
def RNBPG_l0(U0,V0,inst,btau,tau_min,tau_max,sigma,beta,sparse,gap=50,prec=1e-6,stop_rule=0,max_stop=1000):
    ...
    while continua:
        ...
        if ik < r:
            ...
            while adaptive_ls:
                ...
                idui = np.argpartition(sparse_ui, sparse)[:sparse]  # indices of s smallest
                ...
                if norm(di) != 0.0:
                    ...
                    temp_arg = .5*np.dot(di,di)*np.dot(Vik,Vik) + np.dot(gf,di) 
        ...
        # Update V:
            ...
            while adaptive_ls:
                ...
                if norm(di) != 0.0:
                    ...
                    temp_arg = .5*np.dot(di,di)*np.dot(Uik,Uik) + np.dot(gf,di)
