...
output_feval = np.zeros([len(images),3,3])
...
feval_matching_boosted = np.zeros([len(images),3])
...
for ii in range(len(images)):
    ...
    "Nonmonotone:" 
    sol = NMF.RNBPG_btau_adaptive(U0,V0,inst,btau,tau_min,tau_max,M,sigma,beta,gap,prec,stop_rule=0,max_stop=max_stop)
    ...
    output_feval[ii,0,0] = np.round(sol['f_eval'],2)
    ...
    output_feval[ii,0,1] = np.round(solB['f_eval'],2)
    ...
    output_feval[ii,0,2] = np.round(solM['f_eval'],2)
    ...
    feval_matching_boosted[ii,0] = np.round(solF['f_eval'],2)
    ...
    output_feval[ii,1,0] = np.round(sol['f_eval'],2)
    ...
    output_feval[ii,1,1] = np.round(solB['f_eval'],2)
    ...
    output_feval[ii,1,2] = np.round(solM['f_eval'],2)
    ...
    feval_matching_boosted[ii,1] = np.round(solF['F'],2)
    ...
    output_feval[ii,2,0] = np.round(sol['f_eval'],2)
    ...
    output_feval[ii,2,1] = np.round(solB['f_eval'],2)
    ...
    output_feval[ii,2,2] = np.round(solM['f_eval'],2)
    ...
    feval_matching_boosted[ii,2] = np.round(solF['f_eval'],2)
    ...
    IN_im.save('Figures/color/'+sel_image+'_N_R1.jpg')
    
    IBo_im = np.concatenate((IRB_im_0[...,None],IGB_im_0[...,None],IBB_im_0[...,None]),axis=2)
    IBo_im = Image.fromarray(IBo_im)
    IBo_im.save('Figures/color/'+sel_image+'_B_R1.jpg')
    
    IM_im = np.concatenate((IRM_im_0[...,None],IGM_im_0[...,None],IBM_im_0[...,None]),axis=2)
    IM_im = Image.fromarray(IM_im)
    IM_im.save('Figures/color/'+sel_image+'_M_R1.jpg')
    
    np.savez('Results/Results_ExNMF_color_R1', output_iterations, output_time, 
             output_Fval, output_PSNR, it_matching_boosted, time_matching_boosted,
             fval_matching_boosted, PSNR_matching_boosted, output_feval,
             feval_matching_boosted) 
...
output_feval_mean = np.round(np.mean(output_feval,axis=1),2)




fname = "Results/Table_ExNMF_color_R1"
DRNMF.generate_table(fname, images, output_iterations_mean, output_time_mean, output_Fval_mean, output_PSNR_mean, output_feval_mean)
...
# # wr_feval = it_matching_boosted[:,0] / output_iterations[:,0,2]
# # wg_it = it_matching_boosted[:,1] / output_iterations[:,1,2]
# # wb_it = it_matching_boosted[:,2] / output_iterations[:,2,2]
